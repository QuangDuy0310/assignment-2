...
#Test navigation 
def test_navigation_header(driver: WebDriver):
    ...
    logo_element = driver.find_element(By.CSS_SELECTOR, "img[src='/static/images/logo_laptopute.png']")
    logo_element.click()
    time.sleep(2)
    assert "http://127.0.0.1:5000/" in driver.current_url
    account_link = driver.find_element(By.CSS_SELECTOR, "li > a[href='/user-edit-account']") 
    account_link.click()
    time.sleep(2)
    assert "user-edit-account" in driver.current_url
...
def test_navigation_footer(driver):
